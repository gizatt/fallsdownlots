...
def fit_eccentricity(angle_rad: np.ndarray, t_s: np.ndarray) -> tuple[float, float] | None:
    """
    Detrend angle vs time and fit A*sin(theta_mech + phi) to the residual.
    Returns (A, phi) in radians, or None if the fit fails.
    """
    from scipy.optimize import curve_fit
    coeffs = np.polyfit(t_s - t_s[0], angle_rad, 1)
    residual = angle_rad - np.polyval(coeffs, t_s - t_s[0])
    theta = angle_rad % (2 * np.pi)
    try:
        (A, phi), _ = curve_fit(
            lambda th, A, phi: A * np.sin(th + phi),
            theta, residual,
            p0=[residual.std(), 0.0],
...
def figure_sensor_eccentricity(df: pd.DataFrame):
    """
    Diagnose AS5600 eccentricity error.

    Prefers test="eccentricity_cal" (dedicated calibration run); falls back to
    steady-state tails of "velocity_steps".  Detrends each segment, plots the
    angle residual vs mechanical angle, and fits A*sin(theta+phi).

    If eccentricity dominates: curves collapse onto the same sinusoid at all speeds.
    """
    # Prefer dedicated calibration data; fall back to velocity steps.
    if "eccentricity_cal" in df["test"].values:
        sub = df[df["test"] == "eccentricity_cal"].copy()
        # Single long segment — use second half to skip spin-up transient.
        sub = sub.iloc[len(sub) // 4:].reset_index(drop=True)
        segments = [(sub, "eccentricity_cal")]
    else:
        sub = df[df["test"] == "velocity_steps"].copy()
        if sub.empty:
            print("[eccentricity] no usable data found")
            return None
        targets = sorted(sub["step_target"].dropna().unique(), key=abs)
        pos_targets = [t for t in targets if t > 0]
        segments = [
            (sub[sub["step_target"] == tgt].iloc[len(sub[sub["step_target"] == tgt]) // 2:],
             f"{tgt:.0f} rad/s")
            for tgt in pos_targets
        ]

    colors = plt.cm.viridis(np.linspace(0.1, 0.9, max(len(segments), 2)))
    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    fig.suptitle("AS5600 eccentricity diagnosis — angle residual vs mechanical position",
                 fontsize=12)

    fit_A, fit_phi = None, None

    for (seg, label), color in zip(segments, colors):
        if len(seg) < 64:
            continue
        t = seg["t_s"].values
        angle = seg["angle_rad"].values
        coeffs = np.polyfit(t - t[0], angle, 1)
        residual = angle - np.polyval(coeffs, t - t[0])
        mech_angle = angle % (2 * np.pi)

        axes[0].plot(t - t[0], residual, color=color, lw=0.8, alpha=0.7, label=label)

        n_bins = 72
        edges = np.linspace(0, 2 * np.pi, n_bins + 1)
        centers = 0.5 * (edges[:-1] + edges[1:])
        bin_idx = np.clip(np.digitize(mech_angle, edges) - 1, 0, n_bins - 1)
        bin_mean = np.array([
            residual[bin_idx == i].mean() if (bin_idx == i).any() else np.nan
            for i in range(n_bins)
        ])
        axes[1].scatter(mech_angle, residual, s=2, alpha=0.15, color=color, rasterized=True)
        axes[1].plot(centers, bin_mean, color=color, lw=2, label=label)

        result = fit_eccentricity(angle, t)
        rms = np.sqrt(np.mean(residual**2))
        if result:
            A, phi = result
            fit_A, fit_phi = A, phi
            th_fine = np.linspace(0, 2 * np.pi, 360)
            axes[1].plot(th_fine, A * np.sin(th_fine + phi), "k--", lw=1.5,
    ...
    axes[0].axhline(0, color="gray", lw=0.5)
    axes[0].set_xlabel("time (s)")
    axes[0].set_ylabel("angle residual (rad)")
    axes[0].set_title("Detrended angle residual\nsinusoidal → eccentricity")
    axes[0].legend(fontsize=8)
    axes[0].grid(True, alpha=0.3)

    axes[1].axhline(0, color="gray", lw=0.5)
    axes[1].set_xlabel("mechanical angle (rad, mod 2π)")
    axes[1].set_ylabel("angle residual (rad)")
    axes[1].set_title("Residual vs mechanical angle\n"
                      "collapse + sinusoid fit → eccentricity confirmed")
    axes[1].legend(fontsize=8)
    axes[1].grid(True, alpha=0.3)

    fig.tight_layout()
    return fit_A, fit_phi


def figure_cogging_profile(df: pd.DataFrame, n_pole_pairs: int = 7,
                            ecc_A: float = 0.0, ecc_phi: float = 0.0):
    """
    Plot cogging torque profile from low-speed open-loop torque drive.

    Uses test="cogging_cal" if present, otherwise low-speed velocity_steps.
    With eccentricity correction applied (ecc_A, ecc_phi from fit_eccentricity),
    bins velocity vs corrected electrical angle to reveal the cogging pattern.
    """
    if "cogging_cal" in df["test"].values:
        sub = df[df["test"] == "cogging_cal"].copy()
        # Skip spin-up at start.
        sub = sub.iloc[len(sub) // 6:].reset_index(drop=True)
    else:
        sub = df[(df["test"] == "velocity_steps") &
                 (df["step_target"].abs() <= 8.0)].copy()
        if sub.empty:
            print("[cogging_profile] no usable data")
            return
        sub = pd.concat([
            g.iloc[len(g) // 2:] for _, g in sub.groupby("step_target")
        ]).reset_index(drop=True)

    if len(sub) < 64:
        print("[cogging_profile] not enough data")
        return

    angle_raw = sub["angle_rad"].values
    vel_raw = sub["vel_rad_s"].values

    # Apply eccentricity correction to the angle before computing electrical angle.
    # Corrected angle: theta_corr = theta_raw - A*sin(theta_raw + phi)
    angle_corr = angle_raw - ecc_A * np.sin(angle_raw + ecc_phi)
    elec_angle = angle_corr * n_pole_pairs % (2 * np.pi)

    # Mean velocity (proxy for applied torque balance point).
    vel_mean = np.median(vel_raw)
    vel_error = vel_raw - vel_mean

    n_bins = 60
    edges = np.linspace(0, 2 * np.pi, n_bins + 1)
    centers = 0.5 * (edges[:-1] + edges[1:])
    bin_idx = np.clip(np.digitize(elec_angle, edges) - 1, 0, n_bins - 1)
    bin_mean = np.array([
        vel_error[bin_idx == i].mean() if (bin_idx == i).any() else np.nan
        for i in range(n_bins)
    ])
    bin_std = np.array([
        vel_error[bin_idx == i].std() if (bin_idx == i).sum() > 1 else np.nan
        for i in range(n_bins)
    ])

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    ecc_label = f" (ecc corrected: A={ecc_A*1000:.1f} mrad)" if ecc_A != 0 else ""
    fig.suptitle(f"Cogging profile{ecc_label}", fontsize=12)

    axes[0].scatter(elec_angle, vel_error, s=2, alpha=0.15, rasterized=True, color="C0")
    axes[0].plot(centers, bin_mean, color="C1", lw=2, label="bin mean")
    axes[0].fill_between(centers, bin_mean - bin_std, bin_mean + bin_std,
                         alpha=0.25, color="C1")
    axes[0].axhline(0, color="gray", lw=0.5)
    axes[0].set_xlabel(f"electrical angle (rad, mod 2π/{n_pole_pairs} corrected)")
    axes[0].set_ylabel("velocity error (rad/s)")
    axes[0].set_title("Velocity error vs electrical angle")
    axes[0].legend(fontsize=8)
    axes[0].grid(True, alpha=0.3)

    # Spectrum of velocity error — cogging peak should be at speed × pole_pairs / 2π.
    dt = np.median(np.diff(sub["t_s"].values))
    fs = 1.0 / dt
    f, psd = signal.welch(vel_error, fs=fs, nperseg=min(512, len(vel_error) // 4))
    expected_f = vel_mean * n_pole_pairs / (2 * np.pi)
    axes[1].semilogy(f, psd, lw=1.5)
    axes[1].axvline(expected_f, color="r", lw=1.2, ls="--",
                    label=f"expected cogging: {expected_f:.2f} Hz")
    axes[1].set_xlabel("frequency (Hz)")
    axes[1].set_ylabel("PSD [(rad/s)²/Hz]")
    axes[1].set_title("Velocity error spectrum")
    axes[1].legend(fontsize=8)
    axes[1].grid(True, which="both", alpha=0.3)

    peak_err = np.nanstd(bin_mean)
    print(f"[cogging_profile] mean speed={vel_mean:.2f} rad/s  "
          f"cogging vel ripple std={peak_err:.3f} rad/s  "
          f"expected freq={expected_f:.2f} Hz")
...
def main():
    ...
    figures_steps(df)
    figure_bode(df)
    ecc_result = figure_sensor_eccentricity(df)
    ecc_A = ecc_result[0] if ecc_result else 0.0
    ecc_phi = ecc_result[1] if ecc_result else 0.0
    figure_cogging_profile(df, ecc_A=ecc_A, ecc_phi=ecc_phi)
    figure_cogging(df)
